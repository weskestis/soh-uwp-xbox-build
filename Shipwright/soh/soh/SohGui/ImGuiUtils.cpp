...
void RegisterImGuiItemIcons() {
    for (const auto& entry : itemMapping) {
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())
            ->LoadGuiTexture(entry.second.name, entry.second.texturePath, Ship::Color4f(1, 1, 1, 1));
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())
            ->LoadGuiTexture(entry.second.nameFaded, entry.second.texturePath, Ship::Color4f(1, 1, 1, 0.3f));
    }

    for (const auto& entry : gregMapping) {
        Ship::Color4f gregGreen = Ship::Color4f(42.0f / 255.0f, 169.0f / 255.0f, 40.0f / 255.0f, 1.0f);
        Ship::Color4f gregFadedGreen = gregGreen;
        gregFadedGreen.a = 0.3f;
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())
    ...
    for (const auto& entry : actionShuffleMapping) {
        Ship::Color4f aButtonBlue = Ship::Color4f(90.f / 255.f, 90.f / 250.f, 255.f / 255.f, 255.f / 255.f);
        Ship::Color4f aButtonBlueFaded = aButtonBlue;
        aButtonBlueFaded.a = 0.3f;
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())
    ...
    for (const auto& [quest, entry] : songMapping) {
        ...
        Ship::Color4f fadedCol = entry.color;
        fadedCol.a = 0.3f;
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())
    ...
    for (const auto& entry : gSeedTextures) {
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())
            ->LoadGuiTexture(entry.tex, entry.tex, Ship::Color4f(1, 1, 1, 1));
    }
}