...
void SetupMenuElements() {
    ...
    // SetMenu deliberately cannot initialise arbitrary game menus (MM's menu requires a different
    // lifecycle), so initialise OoT's full settings tree here, after CVars have loaded/migrated and
    // every widget has registered. This seeds renderer/audio maps and popout geometry before draw.
    mSohMenu->Init();
}
