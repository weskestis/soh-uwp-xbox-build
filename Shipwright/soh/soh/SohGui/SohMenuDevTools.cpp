...
void SohMenu::AddMenuDevTools() {
    ...
    AddSidebarEntry("Dev Tools", path.sidebarName, 1);
    AddWidget(path, "Unavailable with the SDL3 GPU renderer", WIDGET_TEXT)
        .HideInSearch(true)
        .Options(TextOptions().Color(Colors::Orange));
    AddWidget(path,
              "The legacy display-list debugger depends on the removed OpenGL renderer. Other developer tools remain "
              "available in both graphics modes.",
              WIDGET_TEXT)
        .HideInSearch(true)
