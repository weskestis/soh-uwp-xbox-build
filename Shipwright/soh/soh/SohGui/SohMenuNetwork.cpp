...
void SohMenu::AddMenuNetwork() {
    ...
    path = { "Network", "Unavailable", SECTION_COLUMN_1 };
    AddSidebarEntry("Network", path.sidebarName, 1);
    AddWidget(path,
              "Networking is unavailable in this SDL3 renderer build. Sail, Crowd Control, and Anchor are disabled "
              "because SDL_net currently uses a no-I/O compatibility shim; no setting is silently accepted.",
              WIDGET_TEXT)
        .RaceDisable(false);
    return;
#endif
