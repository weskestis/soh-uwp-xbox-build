...
static const std::map<int32_t, const char*> graphicsModeMap = {
    { ZELDA3D_GRAPHICS_ORIGINAL, "Original SoH" },
    { ZELDA3D_GRAPHICS_OOT3D, "Ocarina of Time 3D" },
};
...
void SohMenu::AddMenuSettings() {
    ...
    AddWidget(path, "Translate Title Screen", WIDGET_CVAR_CHECKBOX)
        ...
        .RaceDisable(false)
        .Options(CheckboxOptions().Tooltip(
            "Translates the original SoH title and any N64 fallback title. OoT3D title assets keep their authored text."));
    ...
    AddWidget(path, "Graphics Pack", WIDGET_SEPARATOR_TEXT);
    AddWidget(path, "Graphics Mode", WIDGET_CVAR_COMBOBOX)
        .CVar(CVAR_SETTING("GraphicsMode"))
        .RaceDisable(false)
        .Callback([](WidgetInfo& info) {
            int mode = CVarGetInteger(CVAR_SETTING("GraphicsMode"), ZELDA3D_GRAPHICS_ORIGINAL);
            if (mode == ZELDA3D_GRAPHICS_OOT3D && !Zelda3D_AssetSourceReady()) {
                mode = ZELDA3D_GRAPHICS_ORIGINAL;
                CVarSetInteger(CVAR_SETTING("GraphicsMode"), mode);
                Notification::Emit({
                    .message = "OoT3D assets unavailable; kept Original SoH. Add oot3d-romfs or a decrypted oot3d.3ds and restart.",
                    .mute = true,
                });
            }
            Zelda3D_RequestGraphicsMode(mode);
            CVarSave();
            if (mode == ZELDA3D_GRAPHICS_OOT3D || Zelda3D_AssetSourceReady()) {
                Notification::Emit({
                    .message = mode == ZELDA3D_GRAPHICS_OOT3D
                                   ? "OoT3D graphics selected; reloading the scene safely"
                                   : "Original SoH graphics selected; reloading the scene safely",
                    .mute = true,
                });
            }
        })
        .Options(ComboboxOptions()
                     .DefaultIndex(ZELDA3D_GRAPHICS_ORIGINAL)
                     .ComboMap(graphicsModeMap)
                     .Tooltip("Switches all available scene, model, animation, texture, camera and collision "
                              "replacements. Missing or damaged OoT3D assets fall back to the original SoH draw. "
                              "When a save is active, the game fades out and reloads the current entrance so 3D "
                              "geometry and collision can never be mixed mid-scene. This is independent of "
                              "Alternate Assets."));
    AddWidget(path, "OoT3D asset source", WIDGET_TEXT)
        .RaceDisable(false)
        .PreFunc([](WidgetInfo& info) {
            if (Zelda3D_AssetSourceReady()) {
                info.name = std::string("OoT3D assets: ready — ") + Zelda3D_AssetSourcePath();
            } else {
                info.name = std::string("OoT3D assets: unavailable — ") + Zelda3D_AssetSourceError();
            }
        });

    AddWidget(path, "OoT3D HD Texture Pack", WIDGET_SEPARATOR_TEXT);
    AddWidget(path, "Use OoT3D HD Texture Pack", WIDGET_CVAR_CHECKBOX)
        .CVar(CVAR_SETTING("OoT3DTexturePackEnabled"))
        .RaceDisable(false)
        .Callback([](WidgetInfo& info) {
            const int enabled = CVarGetInteger(CVAR_SETTING("OoT3DTexturePackEnabled"), 1);
            Zelda3D_TexturePackRequestEnabled(enabled);
            CVarSave();
            Notification::Emit({
                .message = Zelda3D_TexturePackSwitchPending()
                               ? "OoT3D texture-pack switch queued; reloading the scene safely"
                               : Zelda3D_TexturePackStatus(),
                .mute = true,
            });
        })
        .PreFunc([](WidgetInfo& info) {
            info.options->disabled = Zelda3D_TexturePackExternalOverride() != 0;
            info.options->disabledTooltip =
                "ZELDA3D_TEXPACK is an active developer override; remove it to use the saved switch.";
        })
        .Options(CheckboxOptions()
                     .DefaultValue(true)
                     .Tooltip("Uses a compatible Citra/Azahar legacy-hash OoT3D texture pack. The setting is "
                              "independent of Graphics Mode; world replacements appear with Ocarina of Time 3D "
                              "graphics, while shared HUD sources may also use pack art. A live 3D scene reloads "
                              "before the cache changes."));
    AddWidget(path, "OoT3D texture-pack status", WIDGET_TEXT)
        .RaceDisable(false)
        .PreFunc([](WidgetInfo& info) { info.name = Zelda3D_TexturePackStatus(); });
    AddWidget(path, "OoT3D texture-pack install folder", WIDGET_TEXT)
        .RaceDisable(false)
        .PreFunc([](WidgetInfo& info) {
            info.name = std::string("Install: copy the original texture-pack ZIP (no extraction needed), or an ") +
                        "extracted pack folder, into " + Zelda3D_TexturePackInstallDirectory();
        });
    AddWidget(path, "Rescan OoT3D Texture Pack Folder", WIDGET_BUTTON)
        .RaceDisable(false)
        .Callback([](WidgetInfo& info) {
            Zelda3D_TexturePackRequestRescan();
            Notification::Emit({
                .message = Zelda3D_TexturePackSwitchPending()
                               ? "OoT3D texture-pack rescan queued; reloading the scene safely"
                               : Zelda3D_TexturePackStatus(),
                .mute = true,
            });
        })
        .PreFunc([](WidgetInfo& info) {
            info.options->disabled = Zelda3D_TexturePackSwitchPending() != 0;
            info.options->disabledTooltip = "A texture-pack switch or rescan is already pending.";
        })
        .Options(ButtonOptions().Tooltip(
            "Looks for a newly copied ZIP or extracted pack. Invalid packs remain off and report why above."));
    ...
    AddWidget(path, "Anti-aliasing (MSAA)", WIDGET_CVAR_SLIDER_INT)
        ...
        .PreFunc([](WidgetInfo& info) {
            ...
            info.options->disabledTooltip =
                "Unavailable in the current SDL3 GPU renderer; Internal Resolution remains supported.";
        })
        .Callback([](WidgetInfo& info) {
    ...
    AddWidget(path, "Show FPS Overlay", WIDGET_CVAR_CHECKBOX)
        .CVar(CVAR_WINDOW("SohStats"))
        .RaceDisable(false)
        .Options(CheckboxOptions().Tooltip(
            "Shows a live performance window with the current frame rate. Toggle it off to hide the overlay."));
    ...
    AddWidget(path, "Enable Vsync", WIDGET_CVAR_CHECKBOX)
        ...
        .PreFunc([](WidgetInfo& info) {
            info.options->disabled = true;
            info.options->disabledTooltip =
                "Unavailable in the current SDL3 GPU renderer; frame pacing and Match Refresh Rate remain supported.";
        })
        .Options(CheckboxOptions()
    ...
    AddWidget(path, "Allow multi-windows", WIDGET_CVAR_CHECKBOX)
        ...
        .PreFunc([](WidgetInfo& info) {
            info.options->disabled = true;
            info.options->disabledTooltip =
                "Unavailable in the single-window SDL3 GPU/console interface. All menus remain available in-window.";
        })
        .Options(CheckboxOptions()
    ...
    AddWidget(path, "Texture Filter (Needs reload)", WIDGET_CVAR_COMBOBOX)
        ...
        .RaceDisable(false)
        .Options(ComboboxOptions()
                     .Tooltip("Sets filtering for original SoH textures and N64 fallback assets. OoT3D CMB textures "
                              "use their authored material sampler state.")
