...
void Room_Draw(PlayState* play, Room* room, u32 flags) {
    if (room->segment != NULL) {
        ...
        // Zelda3D: suppress the N64 room only after the replacement has synchronously validated.
        // Unmapped, missing, encrypted, or damaged assets deliberately fall through to the original
        // room so graphics mode can never turn the world invisible.
        if (flags & 1) {
