...
void Play_Init(GameState* thisx) {
    ...
    // Commit a requested graphics-mode change only now: the previous PlayState (including its
    // collision) has been destroyed, while this scene has not initialized either renderer yet.
    Zelda3D_ApplyPendingGraphicsMode();
    Zelda3D_TexturePackApplyPending();
...
void Play_DrawOverlayElements(PlayState* play) {
    ...
    // Zelda3D: OoT3D title-demo overlay draw (currently just the logo wordmark, biggest
    // confirmed title-parity gap; see behaviors/title/title_logo.cpp), driven by
    // Zelda3D::TitlePresentation::draw() via this C bridge. No-op outside the title demo.
    Zelda3D_Title_Draw(play);
