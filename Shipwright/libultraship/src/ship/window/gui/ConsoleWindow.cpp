...
void ConsoleWindow::DrawElement() {
    // The Dear ImGui console UI lived here -- ~200 lines of input box, filter combos, log table and
    // autocomplete. It has not rendered since ImGui was removed, and the live console is the REPL
    // plus RmlUi.
    //
    // Everything else in this file STAYS and is live: Dispatch() runs console commands, InitElement
    // registers `help`/`clear`/`bind`/`bindtoggle`, and SendInfoMessage/SendErrorMessage/ClearLogs
    // are called from both games. Only the drawing is gone.
}
