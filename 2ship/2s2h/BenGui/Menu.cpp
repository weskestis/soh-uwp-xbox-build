    ...
    try {
        switch (widget.type) {
            ...
            case WIDGET_WINDOW_BUTTON: {
                ...
                auto window = std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())->GetGuiWindow(widget.windowName);
                if (!window) {
                    std::string msg =
...
void Menu::DrawElement() {
        ...
        BenGui::mModalWindow->RegisterPopup(
            ...
            []() {
                std::shared_ptr<Menu> menu =
                    static_pointer_cast<Menu>(std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())->GetMenu());
    ...
    if (UIWidgets::Button(ICON_FA_UNDO, options2)) {
        std::reinterpret_pointer_cast<Ship::ConsoleWindow>(
            std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())->GetGuiWindow("Console"))
            ->Dispatch("reset");
    }
    ImGui::SameLine();
    ...
    if (UIWidgets::Button(ICON_FA_TIMES_CIRCLE, options3)) {
        ...
        if (CVarGetInteger(CVAR_IMGUI_CONTROLLER_NAV, 0) &&
            std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())->GetMenuOrMenubarVisible()) {
