...
void DrawInGameTimer(uint32_t timer, ImVec4 color = ImVec4(1, 1, 1, 1)) {
    ...
    for (const auto c : timerStr) {
        ...
        if (c == '.') {
            ...
            ImGui::Image(std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())->GetTextureByName(digitList[textureIndex]),
...
void DisplayOverlayWindow::Draw() {
    ...
    ImGui::Image(std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())->GetTextureByName(gTimerClockIconTex),
